...
void bcast2BideThread(DataShared * data,map<int64_t,double> & mapIdf);
void showData(BideThread * bide);
void iniBideThread(BideThread * bide);
void initIdf(map<int64_t,double> & mapIdf);
void * run(void * arg);
...
int master(){
  ...
  map<int64_t,double> mapIdf;
  initIdf(mapIdf);
  bcast2BideThread(pData,mapIdf);
  int64_t count = 0;
  while(nslave>0){
#ifdef _INFO
...
void bcast2BideThread(DataShared * data,map<int64_t,double> & mapIdf){
  ...
  int64_t size = mapIdf.size();
  MPI_Bcast(&(size),sizeof(int64_t),MPI_CHAR,0,MPI_COMM_WORLD);
  for(map<int64_t,double>::iterator iter = mapIdf.begin();
        iter != mapIdf.end();iter++){
    int64_t temp = iter->first;
    double tempidf = iter->second;
    MPI_Bcast(&(temp),sizeof(int64_t),MPI_CHAR,0,MPI_COMM_WORLD);
    MPI_Bcast(&(tempidf),sizeof(double),MPI_CHAR,0,MPI_COMM_WORLD);
  }
...
void iniBideThread(BideThread * bide){
  ...
  for(int64_t i = 0;i < size;i++){
    ...
    MPI_Bcast(&(temp),sizeof(int64_t),MPI_CHAR,0,MPI_COMM_WORLD);
    MPI_Bcast(&(tempidf),sizeof(double),MPI_CHAR,0,MPI_COMM_WORLD);
    bide->m_mapIdf.insert(pair<int64_t,double>(temp,tempidf));
  }
...
void initIdf(map<int64_t,double> & mapIdf){
  ifstream fin("../idf.out");
  if(fin.fail()){
    cerr <<"Can not open idf" <<endl;
  }
  string str;
  while(getline(fin,str)){
    int64_t temp = atoi(str.substr(0,str.find('\t')).c_str());
    double tempidf =atof(str.substr(str.find('\t') + 1).c_str());
    mapIdf.insert(pair<int64_t,double>(temp,tempidf));
  }
}
